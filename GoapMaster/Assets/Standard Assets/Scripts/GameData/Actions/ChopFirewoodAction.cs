...
public class ChopFirewoodAction : GoapAction
{
	...
	public ChopFirewoodAction (Transform tr) : base(tr) {
	...
	public override bool checkProceduralPrecondition (GameObject agent)
	{
		...
		ChoppingBlockComponent[] blocks = (ChoppingBlockComponent[]) GameObject.FindObjectsOfType ( typeof(ChoppingBlockComponent) );
	...
	public override bool perform (GameObject agent)
	{
		...
		if (Time.time - startTime > workDuration) {
			...
			tool.use(0.34f);
			if (tool.destroyed()) {
				GameObject.Destroy(backpack.tool);
				backpack.tool = null;
			}
		}
		return true;
